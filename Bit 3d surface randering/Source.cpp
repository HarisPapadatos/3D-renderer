...
int main()
{
	Camera cam(4.0/3,{20,0,20},{-0.7,-0.2,-0.7});
	cam.setScreenSize(400,300);

	Light light;
	light.position = Vector3d(200,-100, 100);

	while (true) {
		SetConsoleCursorPosition(GetStdHandle(STD_OUTPUT_HANDLE), { 0,0 });
		cam.PrintScreenShot({ SpinningHelix,SpinningHelix2 },light);
		light.position = Vector3d(100*cos(3*t), 100*sin(5*t), 0);
		t += 0.02;
	}
